...
const Handlebars = require("handlebars");
const fs = require("fs");
const expressPhantom = require("phantom-html-to-pdf");
const archiver = require("archiver");
...
// Function to generate PDF using PhantomJS
const generatePhantomPDF = async (htmlString) => {
  const options = {
    phantomPath: {
      phantomPath: require("/Users/bestinepayyappilly/Documents/GitHub/GrogBackend/node_modules/phantomjs-prebuilt/lib/phantomjs.js"),
    }, // Specify the path to your PhantomJS executable
    format: "A4",
    margin: "0",
  };

  const converter = expressPhantom(options);

  try {
    const pdfBuffer = await converter({ html: htmlString });
    return pdfBuffer;
  } catch (error) {
...
app.post("/api/upload_csv", (req, res) => {
  const fileValue = req.files.file.data;
  const csv = Buffer.from(fileValue).toString();
  handleParseCSV(csv);
  res.send({ message: "Received CSV file" });
});
...
app.post("/api/upload-html", async (req, res) => {
  try {
    const fileValue = req.files.file.data;
    const html = Buffer.from(fileValue).toString();

    const pdfData = generatePDF(html);

    const pdfBufferPromises = pdfData.map(async (item, index) => {
      try {
        const phantomPDF = await generatePhantomPDF(item);
        return phantomPDF;
      } catch (error) {
        console.error("Error generating Phantom PDF:", error);
        throw error;
      }
    });
